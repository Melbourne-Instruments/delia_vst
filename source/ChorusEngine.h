...
class ChorusEngine {
  public:
    ...
    void setVol(float vol, bool adj = false) {
        if (!adj) {
            _set_vol = vol;
        }
        _vol = vol;
    ...
    void adjVol(float adj) {
        setVol(std::clamp(_set_vol + adj, 0.f, 1.f), true);
    }
    ...
    inline void process(float *sampleL, float *sampleR, float *out_l, float *out_r) {
        float resultR = 0.0f;
        float resultL = 0.0f;
        if (isChorus1Enabled) {
        ...
        *out_l = resultL * 1.4f * _vol;
        *out_r = resultR * 1.4f * _vol;
    }
