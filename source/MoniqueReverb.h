...
class MoniqueReverb {
  private:
    ...
    float _set_shimmer = 0;
    float _set_wet_dry_mix = 0;
    float _set_predelay = 0;
    float _set_tone_setting = _tone_setting;
    float _set_decay = 0;
  ...
  public:
    ...
    void setShimmer(float val, bool adj = false) {
        if (!adj) {
            _set_shimmer = val;
        }
    ...
    void adjShimmer(float adj) {
        setShimmer(std::clamp(_set_shimmer + adj, 0.f, 1.f), true);
    }

    void setwetDry(float val, bool adj = false) {
        if (!adj) {
            _set_wet_dry_mix = val;
        }
        _wet_dry_mix = val;
    }

    void adjwetDry(float adj) {
        setwetDry(std::clamp(_set_wet_dry_mix + adj, 0.f, 1.f), true);
    }

    void setPreDelay(float val, bool adj = false) {
        if (!adj) {
            _set_predelay = val;
        }
    ...
    void adjPreDelay(float adj) {
        setPreDelay(std::clamp(_set_predelay + adj, 0.f, 1.f), true);
    }

    void setTone(float val, bool adj = false) {
        if (!adj) {
            _set_tone_setting = val;
        }
        _tone_setting = val;
    ...
    void adjTone(float adj) {
        setTone(std::clamp(_set_tone_setting + adj, 0.f, 1.f), true);
    }
    ...
    void setDecay(float val, bool adj = false) {
        if (!adj) {
            _set_decay = val;
        }
    ...
    void adjDecay(float adj) {
        setDecay(std::clamp(_set_decay + adj, 0.f, 1.f), true);
    }

    void setPreset(float preset) {
        uint selection = (uint)preset;
