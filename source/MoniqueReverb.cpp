...
MoniqueReverb::MoniqueReverb(/* args */) {
    input_lpf_0.mute();
    input_lpf_1.mute();
    input_hpf_0.mute();
    input_hpf_1.mute();
...
MoniqueReverb::~MoniqueReverb() {
}

void MoniqueReverb::run(float **inputs, float **final_outputs, uint32_t frames) {
    if (_hard_mute) {
    ...
    case ReverbModes::room:
        ...
        if (bufc % 700 == 0) {
            // printf("\n");
            for (int i = 0; i < 1; i++) {
                // printf(" %f %f %d", late_out_buffer[0][i], inputs[0][i], _hard_mute);
            }
        }
        break;
    ...
    _mix_smooth = Monique::param_smooth(_wet_dry_mix, _mix_smooth);
